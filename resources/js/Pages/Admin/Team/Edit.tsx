...
const Edit = ({ auth, teamData }: PageProps) => {
    const { data, setData, post, errors } = useForm<TeamForm>({
        name: teamData.data.name,
        team_logo: null,
        country: teamData.data.country,
        sport_category: teamData.data.sport_category,
        station: teamData.data.station,
    });
    ...
    const onSubmit = (e: FormEvent<HTMLFormElement>) => {
        ...
        router.post(route("team.update", { team: teamData.data.id }), {
            _method: "patch",
            ...data,
        });
    };
